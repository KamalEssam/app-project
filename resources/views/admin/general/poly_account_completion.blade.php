            ...
            <section>
                ...
                <div class="wizard">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane {{ $active_number == 1 ? 'active ' : (($steps[2]['completed'] == true) ? '' : 'disabled') }}" role="tabpanel" id="step1">
                                ...
                                <div class="text-center">
                                    ...
                                    <ul class="list-unstyled">
                                        @foreach($current_clinics as $current_clinic)
                                            <li>#{{ ++$loop->index }} - {{ $current_clinic->en_name }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                            @endif
                        ...
                        <div class="tab-pane {{ $active_number == 2 ? 'active ' : (($steps[1]['completed'] == true) ? '' : 'disabled')  }}" role="tabpanel" id="step2">
                            {{-- Choose Clinic from DropDown List --}}
                            <div class="row">
                                <div class="col-md-6 col-md-offset-3">
                                    {{ Form::select('clinic_id', \App\Models\Clinic::where('account_id', $auth->account_id)->pluck( app()->getLocale() . '_name' ,'id') ,(isset($_GET['clinic']) && is_numeric($_GET['clinic'])) ? $_GET['clinic'] : null, ['class'=>'form-control ' . ($errors->has('clinic_id') ? 'redborder' : '') , 'required'=>'required', 'id'=>'clinic_id','placeholder' => 'select clinic']) }}
                                </div>
                            </div>
                            <ul class="list-inline pull-right">
                                <li>
...
@push('more-scripts')
    <script>
        ...
        $(document).on('change', '#clinic_id', function (e) {
            window.location = URL + '/poly-account/complete-steps?clinic=' + $(this).val();
        });
    </script>
@endpush
