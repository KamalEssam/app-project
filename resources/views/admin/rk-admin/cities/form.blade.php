...
<div class="row">
      <div class="form-group col-md-12">
        <div class="row">
            <div class="col-md-12 label-form">
                <label for="country_id">{{ trans('lang.country_name') }} <span class="astric">*</span></label>
            </div>
            <div class="col-md-12 form-input">
                {{ Form::select('country_id', $countries->pluck( App::getLocale() . '_name' ,'id') , null,[ 'class'=>'form-control' . ($errors->has('country_id') ? 'redborder' : '') , 'required'=>'required'  , 'id'=>'country_id']) }}
                <small class="text-danger">{{ $errors->first('country_id') }}</small>
            </div>
        </div>
    </div>
</div>
<a href="{{ url('location/cities') }}" class="pull-right loon p-7">{{ trans('lang.cancel') }}</a>
