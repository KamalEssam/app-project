...
@section('title',  trans('lang.manage_provinces') )
...
@section('content')
    <div class="page-content">
        <div class="page-header">
            <div class="row">
                <div class="col-md-11">
                    <h1>{{trans('lang.manage_provinces')}}</h1>
                </div>
                    ...
                    <div class="table-responsive">
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                            ...
                            @foreach($provinces as $province)
                                <?php
                                $created_by = \App\Models\User::where('id', $province->created_by)->first();
                                $updated_by = \App\Models\User::where('id', $province->updated_by)->first();
                                ?>
                                <tr>
                                    <td class="center">{{ $province[app()->getLocale() . '_name']  }}</td>
                                    <td class="center">{{ $province->city[app()->getLocale() . '_name'] }}</td>
                                    ...
                                    <td class="center">
                                        <div class="btn-group control-icon">
                                            <a href="{{ route('provinces.edit', $province->id)  }}"><i
                                                    class="ace-icon fa fa-edit bigger-120  edit"
                                                    title="{{ trans('lang.edit_province') }}"
                                                    data-id=""></i></a>

                                            @can('super-only', $auth)
                                                <a title="{{ trans('lang.delete_province') }}"
                                                   href="#"><i
                                                        class="ace-icon fa fa-trash-alt bigger-120 delete ajax-btn"
                                                        data-id="{{ $province->id }} "
                                                        data-link="{{ route('provinces.destroy', $province->id) }}"
                ...
                @else
                    ...
                    <div class="row">
                        <div class="col-xs-12 text-center"><img class="no_data_image"
                                                                src="{{ asset('assets/images/no_data/no_province.png') }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 text-center"><p
                                class="loon no_data">{{trans('lang.no_provinces')}}</p></div>
                    </div>
